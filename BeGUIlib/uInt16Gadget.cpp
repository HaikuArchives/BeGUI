...
//ctor
UInt16Gadget	::	UInt16Gadget(	uint16 ui16paramInitValue,
								...
								paramMakeFocus)
{
	try
	{
		...
		if (paramLabel)
		{
			...
			myPrefs->GetPreferredUInt16GadgetViewColor(rgbV);
			rgb_color * rgbH = new rgb_color;
			myPrefs->GetPreferredUInt16GadgetHighColor(rgbH);
			rgb_color * rgbL = new rgb_color;
			myPrefs->GetPreferredUInt16GadgetLowColor(rgbL);
	...
	char buf[DIGITSnUI16];
	sprintf(buf, "%u", ui16paramInitValue);
	SetText(buf);
	...
	{
		mfTextViewWidth = WidestDigit() * (DIGITSnUI16 - 1);
	}
...
void 
UInt16Gadget	::	AttachedToWindow(	void)
{
	BTextView::AttachedToWindow();
	rgb_color tempRgbColor;
	myPrefs->GetPreferredUInt16GadgetLowColor(&tempRgbColor);
	SetLowColor(tempRgbColor);
	myPrefs->GetPreferredUInt16GadgetViewColor(&tempRgbColor);
	SetViewColor(tempRgbColor);
	myPrefs->GetPreferredUInt16GadgetHighColor(&tempRgbColor);
...
void 
UInt16Gadget	::	MessageReceived(	BMessage * paramMessage)
{
	const char * entry = Text();
	uint32 len = strlen(entry);//known not utf8
	int32 maxdigits = DIGITSnUI16 - 2;
	if (entry[0] == '+')
	{
		maxdigits++;
	}
